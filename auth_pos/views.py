...
def login(request):
    if request.method == 'POST':
        ...
        if not username or not password:
            messages.error(request, 'Please fill in all fields')
            print("Message added: Please fill in all fields")  # Debugging line
        ...
        if user is not None:
            ...
            print("Message added: Login successful!")  # Debugging line
            ...
            if user_role is None:
                ...
                cache.set(f'user_role_{user.id}', user_role, timeout=60 * 1)  # Cache for 1 minute

            # Redirect immediately after login
            return redirect(f'auth_pos:{user_role}_dashboard')
        else:
            messages.error(request, 'Invalid credentials. Please try again.')
            print("Message added: Invalid credentials.")  # Debugging line
