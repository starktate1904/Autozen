...
def product_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'Price must be a valid number and quantity must be an integer.')
            return render(request, 'products/products.html')
        ...
        try:
            ...
            return redirect('products:product_list')  # Redirect to product list after success
        except IntegrityError:
            messages.error(request, 'Failed to create product. Ensure that the product does not already exist or check your input values.')
            return render(request, 'products/products.html')
...
def product_update(request, product_id):
    ...
    if request.method == 'POST':
        ...
        # Validate input
        if not all([name, make, model, location, price, quantity, branch_id, category_id]):
            messages.error(request, 'Please fill in all fields.')
            return render(request, 'products/products.html', {'product': product, 'active_nav_item': active_nav_item})
        ...
        try:
            ...
            messages.error(request, 'Failed to update product. Ensure that the product does not already exist or check your input values.')
            return render(request, 'products/products.html', {'product': product, 'active_nav_item': active_nav_item})

    # If not POST, render the form with existing product data
