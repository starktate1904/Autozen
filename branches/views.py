...
def branch_create(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'A branch with this name already exists. Please choose a different name.')
            return render(request, 'branches/branches.html', get_context_with_messages(active_nav_item, request))
        except Exception as e:
            messages.error(request, f'An unexpected error occurred while creating the branch: {e}')
            return render(request, 'branches/branches.html', get_context_with_messages(active_nav_item, request))
...
def branch_update(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'Failed to update branch. Ensure that the branch name is unique.')
            return render(request, 'branches/branches.html', get_context_with_messages(active_nav_item, request, {'branch': branch}))
        except Exception as e:
            messages.error(request, f'An unexpected error occurred while updating the branch: {e}')
            return render(request, 'branches/branches.html', get_context_with_messages(active_nav_item, request, {'branch': branch}))

    # Render the branch update form with existing data if not POST
    return render(request, 'branches/branches.html', get_context_with_messages(active_nav_item, request, {'branch': branch}))
